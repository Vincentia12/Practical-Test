		...
		<?php foreach ($produk as $brg) : ?>
			...
			<div class="card mx-auto mb-2" style="width: 14rem;">
				...
				<div class="card-body">
					...
					<span class="badge badge-pill badge-warning mb-1">Rp. <?php echo number_format($brg->harga, 0, ',', '.') ?></span>
					<?php echo anchor('dashboard/masukkan_keranjang/' . $brg->id_produk, '<div class="btn btn-sm btn-primary mt-2")>Masukkan Keranjang</div>') ?>
					<?php echo anchor('dashboard/detail/' . $brg->id_produk, '<div class="btn btn-sm btn-success mt-2")>Detail</div>') ?>
				</div>
			</div>
